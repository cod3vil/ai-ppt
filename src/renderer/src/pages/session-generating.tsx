  ...
  return (
    <div className="relative flex h-full flex-col overflow-hidden bg-white">
      <style>{`
        ...
        @keyframes gen-bg-pulse {
          0%, 100% { opacity: 0.35; transform: translate(0, 0) scale(1); }
          50% { opacity: 0.6; transform: translate(2%, -1%) scale(1.05); }
        }
      `}</style>

      {/* Soft ambient loading aura — replaces the rabbit video background */}
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 z-0 overflow-hidden bg-white"
      >
        <div
          className="absolute -left-32 -top-40 h-[520px] w-[520px] rounded-full bg-[radial-gradient(circle,rgba(124,58,237,0.18)_0%,rgba(124,58,237,0)_70%)]"
          style={{ animation: 'gen-bg-pulse 8s ease-in-out infinite' }}
        />
        <div
          className="absolute -right-40 bottom-[-12rem] h-[600px] w-[600px] rounded-full bg-[radial-gradient(circle,rgba(124,58,237,0.12)_0%,rgba(124,58,237,0)_70%)]"
          style={{ animation: 'gen-bg-pulse 11s ease-in-out infinite', animationDelay: '-3s' }}
        />
      </div>

      <div className="app-drag-region app-titlebar relative z-20 flex items-center bg-white/90 backdrop-blur-sm" />
