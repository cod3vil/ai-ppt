...
export function GenerationStatusPanel({
  ...
  if (status === 'failed') {
    return (
      <div className="mb-4 shrink-0 rounded-lg border border-[#fecaca]/80 bg-[#fef2f2]/86 px-4 py-2.5 text-[var(--color-danger)] shadow-[0_8px_20px_rgba(120,73,65,0.08)]">
        <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex min-w-0 items-center gap-2.5">
            <CircleAlert className="h-4 w-4 shrink-0" />
            <span className="shrink-0 rounded-md border border-[#fecaca]/70 bg-[#fef2f2]/75 px-2 py-1 text-xs font-semibold text-[var(--color-danger)]">
              {interruptedLabel}
            </span>
            ...
            {canEnterEditor && (
              <button
                ...
                className="shrink-0 text-xs font-medium text-[var(--color-brand)] underline-offset-2 hover:underline"
              >
                {enterEditorLabel}
              </button>
            )}
  ...
  return (
    <div className="mb-4 shrink-0 rounded-lg border border-[var(--color-border-default)] bg-[#ffffff] px-4 py-2 text-[#435138] shadow-[0_12px_28px_rgba(78,91,63,0.13)]">
      <div className="flex flex-col gap-2 lg:flex-row lg:items-end lg:gap-4">
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] text-[#617350]">
            <span className="flex min-w-0 flex-wrap items-center gap-2 text-[10px] text-[var(--color-fg-tertiary)]">
                ...
                return (
                  <span
                    ...
                    className={cn(
                      ...
                      !isDone && !isActive && 'text-[var(--color-fg-tertiary)]'
                    )}
                  >
          ...
          <div className="h-1.5 overflow-hidden rounded-full border border-[var(--color-border-default)]/80 bg-[#ffffff] shadow-[inset_0_1px_2px_rgba(74,58,40,0.12)]">
            <div
              className="h-full rounded-full bg-[linear-gradient(90deg,#9ecf8a_0%,#6f9f59_52%,var(--color-fg-secondary)_100%)] bg-[length:200%_100%] transition-[width] duration-500"
              style={{
