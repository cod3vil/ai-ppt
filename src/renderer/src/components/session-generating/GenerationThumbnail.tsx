...
export function GenerationThumbnail({
  ...
  return (
    <div
      className={cn(
        'group relative overflow-hidden rounded-xl border bg-[#ffffff]/78 p-2 shadow-[0_16px_34px_rgba(70,82,58,0.12)] transition-all duration-500',
        ...
        page.status === 'failed' && 'border-[#fecaca] bg-[#fef2f2]/92',
        page.status === 'pending' && 'border-[#dfd4bf]/72 opacity-72'
      )}
    >
      <div className="relative aspect-video overflow-hidden rounded-lg border border-[var(--color-border-default)]/70 bg-[#efe6d6]">
        {hasPreview ? (
          <PreviewIframe
          ...
          <div
            className={cn(
              ...
              page.status === 'generating'
                ...
                : page.status === 'failed'
                  ? 'bg-[#f7e7e2]'
                  : 'bg-[linear-gradient(135deg,var(--color-bg-subtle)_0%,#e9decb_100%)]'
            )}
          >
      ...
      <div className="mt-2 flex items-center justify-between gap-2">
        <span className="shrink-0 rounded-md bg-[var(--color-brand)]/10 px-1.5 py-0.5 text-[10px] font-semibold text-[var(--color-brand-active)]">
          P{page.pageNumber}
        </span>
        {page.status !== 'failed' && (
