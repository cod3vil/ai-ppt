...
export function LayoutInspector({ draft }: ElementEditorProps): React.JSX.Element {
  ...
  return (
    <InspectorSection
      ...
      icon={<Move className="h-3.5 w-3.5 text-[var(--color-fg-tertiary)]" />}
    >
      <div className="grid grid-cols-4 gap-2">
        {LAYOUT_FIELDS.map(({ key, label }) => (
          <div key={key} className="space-y-1 text-center">
            <span className="text-[11px] font-medium text-[var(--color-fg-tertiary)]">{label}</span>
            <div className="flex h-8 items-center justify-center rounded-full border border-[var(--color-border-default)]/40 bg-[var(--color-bg-subtle)]/40 px-1.5 text-[11px] text-[#a0977e]/70">
              {draft[key]}
            </div>
          </div>
        ))}
